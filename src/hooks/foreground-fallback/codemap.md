...
- Per-session state:
  ...
  - `v2RetryTerminal`: sessions the v2 in-place retry hook has put into a terminal state (permanent quota/billing exhaustion). Later retry-hook calls answer `{ retry: false }` until a completed successful assistant response, a genuine new turn returning to the configured primary, or session deletion/dispose clears it — kept in lock-step with `chainExhaustion`.
...
- The v2 in-place retry hook shares the chain-global budget and permanent-quota exhaustion policy. Absorbed retries, recoverable failures, missing chains and unsuccessful model switches leave the host decision unchanged. When `selectFallbackModel` returns `exhausted` for an `isPermanentUsageQuotaError`, the hook records a per-session terminal reason and sets `event.decision = { retry: false }`; every later retry-hook call on that session answers `{ retry: false }` before classification, delay and model selection. The reason clears on a completed successful assistant response (in lock-step with stage 2), on `session.deleted`, `dispose`, and only when a genuine new user turn actually reopens stage 2 at the configured primary.
...
- `freshTurnResetHandler` runs on any confirmed new `user` turn (the SDK `UserMessage` nests the model under `info.model`; assistant messages keep it top-level): it cancels a pending initial-delay trigger and clears the budget, retry episode, `sessionTried`, dedup anchors and `lastFallbackTime`. A user message is only treated as a real turn when it is NOT inside our own in-flight replay and NOT a message id already confirmed (or positively identified) as our own replay (`handleUserTurn`: a retained `replayMessageIds` hit, a baseline message id match, or an internal-initiator part confirmed from the transcript tail). A confirmed replay id is retained, and an in-flight transcript read deletes `pendingReplay` only if it is still the same record it examined. Un-sealing the stage-2 terminal guard additionally requires the turn to return to `chain[0]`.
